...
void OctomapGenerator<PCLSemanticsMax, SemanticsOctreeMax>::updateColorAndSemantics(PCLSemanticsMax *pcl_cloud)
{
       ...
       ++it)
  {
    ...
    if (!std::isnan(coord.x()) && !std::isnan(coord.y()) && !std::isnan(coord.z()))
    {
      ...
      // Don't save freespace information
      // TODO decide if this is the right choice
      if (octomap_.isNodeOccupied(node))
      {
        auto color = node->getColor();
        auto semantic_color = node->getSemantics().semantic_color;
        auto semantic_conf = node->getSemantics().confidence;
        outfile << coord.x() << ", " << coord.y() << ", " << coord.z() << ", ";
        outfile << static_cast<int>(color.r) << ", " << static_cast<int>(color.g) << ", " << static_cast<int>(color.b) << ", ";
        outfile << static_cast<int>(semantic_color.r) << ", " << static_cast<int>(semantic_color.g) << ", " << static_cast<int>(semantic_color.b) << ", ";
        outfile << semantic_conf << std::endl;
      }
    }
  }
  outfile.close();
